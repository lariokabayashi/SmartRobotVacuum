...
public class AppInput implements RobotObserver
{
    ...
    private final List<AppRemoteObserver> remoteObservers = new ArrayList<>();
    ...
    public void registerRemoteObserver(AppRemoteObserver observer) {
        remoteObservers.add(observer);
    }
    
    private void notifyRemoteObservers(Map map, float percent) {
        List<AppRemoteObserver> failedObservers = new ArrayList<>();
        for (AppRemoteObserver obs : remoteObservers) {
            try {
                obs.updateMap(map, percent);
            } catch (RemoteException e) {
                System.err.println("Failed to notify remote observer: " + e.getMessage());
                failedObservers.add(obs);
            }
        }
        // Remove failed observers
        remoteObservers.removeAll(failedObservers);
    }
